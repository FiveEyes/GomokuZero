...
class SmartServer(object):
    ...
    def init_workers(self):
        ...
        playbook = config.server_config['playbook']
        for i in range(min(len(self.workers), len(playbook))):
            self.workers[i].set_moves(playbook[i])
            
        #self.workers[0].play_style = 1
        #self.workers[1].play_style = 2
        #self.workers[0].set_moves([112, 128, 126])
        #self.workers[1].set_moves([112, 97, 96])
