...
class MCTSNode:
    ...
    # TD update
    def get_Q(self):
        if self.N <= 1 or self.is_leaf() or self.winner != 0:
            if self.winner != 0:
                print("Q", "winner", self.winner, self.Q)
            return self.Q
        
        mv, node = max(self.children.items(), key=lambda mv_node: mv_node[1].N)
        node_prob = node.N / (self.N - 1.0)
        print("Q", self.Q, self.N, node.N, node_prob, node.Q)
    ...
    def backup_winner(self):
        ...
        if self.parent is None or self.parent.winner != 0:
            return
